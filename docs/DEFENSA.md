...
*(`docs/LIMITACIONES.md` fusionado en esta sección — un solo lugar para "qué no hicimos
y por qué", evitar la misma pregunta respondida en dos archivos.)*

### 7.1 Limitaciones reconocidas (fuera del alcance del MVP)
...
- Se conserva el estado actual, no una historia de transiciones con fecha,
  responsable y motivo. Tampoco hay motivo de rechazo persistido — ver
  `docs/MEJORAS.md` #172.

**Otras limitaciones puntuales**
...
### 7.2 Fuera de alcance — GPS y geolocalización interactiva
...
3. **Validación server-side:** el punto lat/lng, si viene, debe caer dentro
   ...
   si cae fuera.
4. **Documentación:** actualizar la sección 1.4 de la tesis explicando el
   cambio de alcance y su justificación técnica.
...
1. **Piloto con usuarios reales** — ciudadanos y organizaciones de
   ...
   encontrarla, interpretar su estado, gestionarla. Instrumento ya
   preparado en `docs/USABILIDAD.md` (Likert por funcionalidad),
